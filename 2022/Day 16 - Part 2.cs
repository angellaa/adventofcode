...
int MaxFlow(int time, int totalFlow, Valve myValve, Valve elephantValve, int myMovesWithoutOpens, int elephantMovesWithoutOpens)
{
    if (myVisited.Contains(myValve) || 
        ...
        myMovesWithoutOpens + elephantMovesWithoutOpens > 7) //||
//        (time > 19 && totalFlow < best * 0.5))//+ unopenedValves.Sum(x => x.FlowRate) * (26 - time) < best))
    {
...
double Rank(string name) => explorationSpace.Except(openValves.Select(x => x.Name)).Sum(p => valves.Find(x => x.Name == p).FlowRate * (26 - distance[(name, p)]));
...
{    
    ...
    public List<Valve> Next(List<Valve> valves, Dictionary<string, double> rank)
    {
        if (_nextValves == null)
        {
            _nextValves = NextValves.Select(x => valves.Find(v => v.Name == x)).OrderByDescending(x => rank[x.Name]).ToList();
        }
